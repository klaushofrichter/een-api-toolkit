...
test.describe('Bridges Example App', () => {
  test('should display the home page', async ({ page }) => {
    await page.goto('/')

    // Check title
    await expect(page).toHaveTitle('EEN Bridges Example')

    // Check header
    await expect(page.locator('h1')).toContainText('EEN Bridges Example')
  ...
  test('should show welcome message on home page', async ({ page }) => {
    ...
    await expect(page.locator('h2')).toContainText('Welcome to the EEN Bridges Example')
  })
  ...
  test('should display login prompt when not authenticated', async ({ page }) => {
    ...
    await expect(page.locator('.login-prompt')).toContainText('Please log in to view your bridges')
  })
  ...
  test('should redirect to login when accessing bridges without auth', async ({ page }) => {
    // Try to access bridges directly
  ...
  test('should show about section on home page', async ({ page }) => {
    ...
    await expect(page.locator('.description')).toContainText('getBridges')
    await expect(page.locator('.description')).toContainText('getBridge')
  })
