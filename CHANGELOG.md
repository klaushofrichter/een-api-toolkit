...
- **Node engines floor raised to `>=20.19.0`** (was `>=20.0.0`) to match the
  Node version required by the build/test toolchain (vite 8 / rolldown, jsdom).
- Build tooling: upgraded `vite-plugin-dts` 4.x → 5.x (migrated `rollupTypes`
  → `bundleTypes`; added `@microsoft/api-extractor` and `@vue/language-core`
  as dev dependencies). The published declaration bundle (`dist/index.d.ts`)
  is regenerated by the new toolchain; the runtime output (`index.js`/`.cjs`)
  is unchanged. `verify-package.sh` now type-checks the bundled declaration
  in CI to guard against dangling type references.
