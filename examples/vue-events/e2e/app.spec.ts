...
test.describe('Events Example App', () => {
  test('should display the home page', async ({ page }) => {
    await page.goto('/')

    // Check title
    await expect(page).toHaveTitle('EEN Events Example')

    // Check header
    await expect(page.locator('h1')).toContainText('EEN Events Example')
  ...
  test('should show welcome message on home page', async ({ page }) => {
    ...
    await expect(page.locator('h2')).toContainText('Welcome to the EEN Events Example')
  })
  ...
  test('should display login prompt when not authenticated', async ({ page }) => {
    ...
    await expect(page.locator('.login-prompt')).toContainText('Please log in to view camera events')
  })
  ...
  test('should show about section on home page', async ({ page }) => {
    ...
    await expect(page.locator('.description')).toContainText('listEvents')
    await expect(page.locator('.description')).toContainText('listEventTypes')
    await expect(page.locator('.description')).toContainText('listEventFieldValues')
  })
