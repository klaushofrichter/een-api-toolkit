...
test.describe('vue-feeds example app', () => {
  test('home page shows login button when not authenticated', async ({ page }) => {
    await page.goto('/')

    // Should show the home page
    await expect(page.getByRole('heading', { name: 'Welcome to the EEN Feeds Example' })).toBeVisible()
    ...
    await expect(page.getByText('Please log in to view camera feeds')).toBeVisible()
  })
  ...
  test('feeds route redirects to login when not authenticated', async ({ page }) => {
  ...
  test('about section displays toolkit function list', async ({ page }) => {
    ...
    await expect(page.getByText('listFeeds()')).toBeVisible()
  })
})
